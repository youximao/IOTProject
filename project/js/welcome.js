    ...
    $.get("../cgi/chao_wsnserial.cgi",{},function(data){
     ...
     // 关闭放大
        if(oldLength == 1){
                    ...
                    $("#autolightId").trigger("mouseleave");
                }
        ...
        var procsee=length*1440/8
         $("#lightProcessId").css("width",procsee+"px");
          ...
          // "led.cgi？"+length
           $.get("../cgi/cgi_main.cgi?"+length,{},function(data){
                                 $("#testChangeId").val(data+i);
                             });
        }
		
      function autolightCon() {

          // ./php/test.php 是相对的url显示 改成 guangzhao.cgi? + length
          // "guangzhao.cgi？"+length
          $.get("../led_wanserial.cgi?"+length,{},function(data){
              $("#testChangeId").val(data+i);
          });
      ...
      $.get("../cgi/cgi_main.cgi?9",{},function(data){
                        $("#testChangeId").val(data+i);
                    });
    ...
    setInterval("testLunXun()",1000);
     ...
     $("#changeLightId").bind("click",function (){
         ...
         $("#lightProcessId").css("width",length+"px");
          ...
          // 进行控制的代码
         $.get("/cgi/cgi_main.cgi",{},function(data){
                  $("#testChangeId").val(data+i);
              });
        ...
        $("#autolightId").bind("mouseover",function(){
            $("#autolightId").addClass("butbig");
        });

        $("#autolightId").bind("mouseleave",function(){
            $("#autolightId").removeClass("butbig");
        });
...
app.controller('things',function($scope,$http,$interval){
 ...
 var config = { headers:{"content-type":"text/html"}};
   ...
   $scope.lightBig=function () {
    ...
    $scope.autolightBig=function () {
        $("#autolightId").addClass("butbig");
    }
    $scope.autolightSmall=function () {
        $("#autolightId").removeClass("butbig");
    }
